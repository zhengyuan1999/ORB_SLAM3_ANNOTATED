            ...
            if (!vIndices.empty())
            {
                ...
                for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++)
                {
                    ...
                    if (dist < bestDist)
                    {
                        ...
                        bestLevel =
                            (F.Nleft == -1) ? F.mvKeysUn[idx].octave : (idx < F.Nleft) ? F.mvKeys[idx].octave : F.mvKeysRight[idx - F.Nleft].octave;
                        bestIdx = idx;
                    }
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    ...
    // const float factor = 1.0f / HISTO_LENGTH; // 这里写错了！
    ...
    for (int i = 0; i < LastFrame.N; i++)
    {
        ...
        int nLastOctave =
            (LastFrame.Nleft == -1 || i < LastFrame.Nleft) ? LastFrame.mvKeys[i].octave : LastFrame.mvKeysRight[i - LastFrame.Nleft].octave;
        ...
        if (bForward) // bForward = tlc(2) > CurrentFrame.mb && !bMono;
        {
            vIndices2 = CurrentFrame.GetFeaturesInArea(uv(0), uv(1), radius, nLastOctave);
        }
        else if (bBackward) // bBackward = -tlc(2) > CurrentFrame.mb && !bMono;
        {
...
int ORBmatcher::SearchByProjection(KeyFrame *pKF, Sophus::Sim3<float> &Scw, const vector<MapPoint *> &vpPoints, vector<MapPoint *> &vpMatched, int th, float ratioHamming)
{
            ...
            for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++)
            {
                ...
                for (size_t iF = 0; iF < vIndicesF.size(); iF++)
                {
                    if (F.Nleft == -1)
                    {
                        ...
                        {
                            // F 的第 realIdxF 个特征点已经有了路标点匹配
                            // TODO 万一另一个路标点更适合这个特征点呢？是不是应该照着 ORBmatcher::SearchForInitialization() 改一下？
                            continue;
                        }
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++)
    {
        ...
        int bestDist  = INT_MAX; // 最优描述子距离
        int bestDist2 = INT_MAX; // 次优描述子距离
        int bestIdx2 = -1;       // 具有最优描述子距离的 F2 描述子索引
        ...
        for (vector<size_t>::iterator vit = vIndices2.begin(); vit != vIndices2.end(); vit++)
        {
            ...
            {
                continue; // F2 第 i2 个特征点已经具有更好的匹配
            }
        ...
        if (bestDist <= TH_LOW)
        {
            if (bestDist < (float)bestDist2 * mfNNratio)
            {
                ...
                {
                    // 对于 F1.mvKeysUn[i1] 具有最优匹配的 F2.mvKeysUn[bestIdx2] 已经被匹配给 F1 的其他特征点了
                    // 但是，如果能运行到这里，说明 vMatchedDistance[i2] 一定大于 dist，F2.mvKeysUn[bestIdx2] 更加匹配 F1.mvKeysUn[i1]
                    vnMatches12[vnMatches21[bestIdx2]] = -1; // 把 F2.mvKeysUn[bestIdx2] 之前的匹配在 vnMatches12 置 -1
                    nmatches--;
                }
