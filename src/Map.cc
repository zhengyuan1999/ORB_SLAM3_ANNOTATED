...
{
    mnId = nNextId++;
    // mThumbnail = static_cast<GLubyte *>(NULL); // 没用到
}
...
void Map::ApplyScaledRotation(const Sophus::SE3f &T, const float s, const bool bScaledVel)
{
    ...
    Sophus::SE3f Tyw = T; // gw = Rwg * g = Rwg * (0, 0, -IMU::GRAVITY_VALUE)^T
    ...
    for (set<KeyFrame *>::iterator sit = mspKeyFrames.begin(); sit != mspKeyFrames.end(); sit++)
    {
        ...
        */
        ...
        Twc.translation() *= s; // 恢复尺度
        Sophus::SE3f Tyc = Tyw * Twc;
        Sophus::SE3f Tcy = Tyc.inverse();
        pKF->SetPose(Tcy);

        Eigen::Vector3f Vw = pKF->GetVelocity();
        if (!bScaledVel) pKF->SetVelocity(Ryw * Vw);
        else pKF->SetVelocity(Ryw * Vw * s);
    }
    ...
    {
        MapPoint *pMP = *sit;
        pMP->SetWorldPos(s * Ryw * pMP->GetWorldPos() + tyw); // tyw 始终为 0 向量
