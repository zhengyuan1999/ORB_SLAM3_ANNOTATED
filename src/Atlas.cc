...
Atlas::Atlas(int initKFid) : mnLastInitKFidMap(initKFid)//, mHasViewer(false) // mHasViewer 没用到
{
    mpCurrentMap = static_cast<Map *>(NULL);
    CreateNewMap();
}
...
void Atlas::CreateNewMap()
{
    ...
    if (mpCurrentMap) // 活跃地图（当前地图）有效，则先将当前活跃地图存储为非活跃地图
    {
        ...
        mpCurrentMap->SetStoredMap(); // 将活跃地图设置为非活跃地图
    ...
    mpCurrentMap = new Map(mnLastInitKFidMap); // 新建一张地图对象
    mpCurrentMap->SetCurrentMap();             // 将新地图设置为活跃地图
