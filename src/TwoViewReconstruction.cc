...
{
// Step1. 对 mvKeys1、mvKeys2、mvMatches12 和 mvbMatched1 赋值
    mvKeys1.clear();
    mvKeys2.clear();
...
// Step2. 选取 RANSAC 集合（共计 mMaxIterations 个集合元素，每个集合元素 8 个点对）
    const int N = mvMatches12.size();

    // Indices for minimum set selection
    vector<size_t> vAllIndices; // mvMatches12 所有的索引：1, 2, 3, ..., N - 1
    vAllIndices.reserve(N);
    vector<size_t> vAvailableIndices; // 每次选取 8 点对时所有剩余可选点对的索引（不能选择重复的点对）

    for (int i = 0; i < N; i++)
    {
        vAllIndices.push_back(i); // 填充 vAllIndices：1, 2, 3, ..., N - 1
    }
    ...
    DUtils::Random::SeedRandOnce(0); // srand(seed);
...
// Step3. 多线程计算基础矩阵和和单应矩阵（Launch threads to compute in parallel a fundamental matrix and a homography）
    vector<bool> vbMatchesInliersH, vbMatchesInliersF; // 对于计算出的两个矩阵而言是否为内点
    float SH, SF; // 两个矩阵的评分
    Eigen::Matrix3f H, F; // 保存这两个矩阵
    ...
    // 等待上面两个进程结束（Wait until both threads have finished）
    threadH.join();
    threadF.join();
...
void TwoViewReconstruction::FindFundamental(vector<bool> &vbMatchesInliers, float &score, Eigen::Matrix3f &F21)
{
    ...
    for (int it = 0; it < mMaxIterations; it++)
    {
        // 选取一个 RANSAC 集合元素，也就是 8 点对（Select a minimum set）
        for (int j = 0; j < 8; j++)
        {
            int idx = mvSets[it][j];
        ...
        /**
         * 0 = vPn2i^T * Fn * vPn1i
         * 0 = (T2 * mvMatches12[idx].second)^T * Fn * (T1 * mvMatches12[idx].first)
         * 0 = (mvMatches12[idx].second)^T * (T2t * Fn * T1) * mvMatches12[idx].first
         * F21i = T2t * Fn * T1
        */
        F21i = T2t * Fn * T1; // 解归一化
...
{
    const int N = vP1.size(); // vP1.size() == vP2.size() == 8
    ...
    // 对系数矩阵 A 进行奇异值分解
    // Eigen::JacobiSVD<Eigen::MatrixXf> svd(A, Eigen::ComputeFullU | Eigen::ComputeFullV); // 没必要填充 U 矩阵
    Eigen::JacobiSVD<Eigen::MatrixXf> svd(A, Eigen::ComputeFullV);
...
float TwoViewReconstruction::CheckHomography(const Eigen::Matrix3f &H21, const Eigen::Matrix3f &H12, vector<bool> &vbMatchesInliers, float sigma)
{
    ...
    const float th = 5.991; // 2 自由度卡方分布 P 值为 0.05
...
float TwoViewReconstruction::CheckFundamental(const Eigen::Matrix3f &F21, vector<bool> &vbMatchesInliers, float sigma)
{
    ...
    const float th = 3.841;      // 1 自由度卡方分布 P 值为 0.05
    const float thScore = 5.991; // 2 自由度卡方分布 P 值为 0.05

    const float invSigmaSquare = 1.0 / (sigma * sigma); // 协方差矩阵的逆矩阵
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        // squareDist1 就是点 x2 到直线 a2 * x + b2 * y + c = 0 的距离，这里认为距离只有 1 自由度
        const float squareDist1 = num2 * num2 / (a2 * a2 + b2 * b2);

        const float chiSquare1 = squareDist1 * invSigmaSquare;

        // TODO 这里为什么用 1 自由度判断，用 2 自由度评分？为了和单应矩阵评分呼应？
        if (chiSquare1 > th)
            bIn = false;
        else
...
{
    /**
     * meanX = ∑(vKeys[i].pt.x) / N
     * 
     * meanDevX = ∑(vKeys[i].pt.x - meanX) / N
     * 
     *     | sX  0 -meanX * sX |   | 1.0 / meanDevX               0 -meanX / meanDevX |
     * T = |  0 sY -meanY * sY | = |              0  1.0 / meanDevY -meanX / meanDevX |
     *     |  0  0         1.0 |   |              0               0               1.0 |
     * 
     * | vNormalizedPoints[i].x |   | (vKeys[i].pt.x - meanX) * sX |   | sX  0 -meanX * sX |   | vKeys[i].pt.x |
...
int TwoViewReconstruction::CheckRT(const Eigen::Matrix3f &R, const Eigen::Vector3f &t, const vector<cv::KeyPoint> &vKeys1,
