...
{
// Step1. 打印欢迎信息（Output welcome message）
    cout << endl
    ...
    else if (mSensor == IMU_MONOCULAR) cout << "Monocular-Inertial" << endl;
    else if (mSensor == IMU_STEREO) cout << "Stereo-Inertial" << endl;
    else if (mSensor == IMU_RGBD) cout << "RGB-D-Inertial" << endl;


// Step2. 检查（Check settings file）并读取配置文件
    ...
    else // 这里为什么没有 File.version: "1.0" 就不加载配置文件了？将 File.version: "1.0" 注释掉程序段错误
    {
        settings_ = nullptr;
    ...
    // 创建关键帧数据库对象（Create KeyFrame Database）
    mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);

    if (mStrLoadAtlasFromFile.empty()) // 配置文件中未给定加载地图路径
    {
        // if else 中的这段代码一样，不如放到外边
        // // Load ORB Vocabulary
        // cout << endl << "Loading ORB Vocabulary. This could take a while..." << endl;
        // mpVocabulary = new ORBVocabulary();
        // bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile);
...
// Step3. 创建并启动各个线程
    // 创建帧绘制器和地图绘制器，这两个对象将被可视化线程使用（Create Drawers. These are used by the Viewer）
    mpFrameDrawer = new FrameDrawer(mpAtlas);
    mpMapDrawer = new MapDrawer(mpAtlas, strSettingsFile, settings_);

    // 创建追踪对象，相当于启动了追踪线程（Initialize the Tracking thread），该线程生存在主线程中
    ...
    mpTracker = new Tracking(
        ...
        strSettingsFile,    // 配置文件路径对象
        mSensor,            // 输入传感器类型
        settings_,          // 配置指针
        strSequence         // TODO 不知道干嘛的？
    );

    // 创建局部建图对象并启动局部建图线程（Initialize the Local Mapping thread and launch）
    mpLocalMapper = new LocalMapping(
        this,                                                                     // SLAM 系统指针
        mpAtlas,                                                                  // 地图指针
        mSensor == MONOCULAR || mSensor == IMU_MONOCULAR,                         // 输入传感器类型是否为单目
        mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD, // 输入传感器类型是否为 IMU
        strSequence                                                               // 
    );
    ...
    // 建立三个线程（追踪线程、局部建图线程和回环检测线程）的关联（Set pointers between threads）
    ...
    // usleep(10*1000*1000); 睡啥？嗯？

    // 创建可视化对象并启动可视化线程（Initialize the Viewer thread and launch）
    if (bUseViewer)
    // if(false) // -TODO 为了不和自己的 TODO 冲突在原作者 TODO 前面加了个 -
    {
        mpViewer = new Viewer(
            this,            // SLAM 系统指针
            mpFrameDrawer,   // 帧绘制器指针
