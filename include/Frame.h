...
public:
    ...
    // 复制构造方法（Copy constructor.）
    Frame(const Frame &frame);

    // 双目帧构造方法（Constructor for stereo cameras.）
    ...
    // RGB-D 帧构造方法（Constructor for RGB-D cameras.）
    ...
    // 单目帧构造方法（Constructor for Monocular cameras.）
    ...
    /**
     * @brief 在已知关键点深度的情况下，将该关键点反向投影到世界坐标系下（Backprojects a keypoint
     * (if stereo/depth info available) into 3D world coordinates.）
     * 
     * @param[in] i mvKeysUn 的索引
     * @param[out] x3D 反向投影出来的世界坐标系下的点
     * 
     * @return 只有深度为正时才可反向投影，返回 true
    */
...
public:
    ...
    cv::Mat mK;          // 相机内参矩阵（cv::Mat 版本）
    Eigen::Matrix3f mK_; // 相机内参矩阵（Eigen::Matrix 版本）
    static float fx;
    static float fy;
    static float cx;
    static float cy;
    ...
    int N; // 关键点数量（Number of KeyPoints.）
    ...
    static long unsigned int nNextId; // 下一帧的帧 ID
    long unsigned int mnId;           // 本帧的帧 ID

    KeyFrame *mpReferenceKF; // 本帧的参考关键帧（Reference Keyframe.）
